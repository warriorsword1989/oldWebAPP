...
App.layersConfig = [{
    ...
    layers: [{
        ...
        url: createUrl('/render/obj/getByTileWithGap?', 'RDRESTRICTION,RDSPEEDLIMIT,RDBRANCH,RDCROSS,RDLANECONNEXITY,RDGSC'),
        clazz: fastmap.mapApi.tileJSON,
        options: {
            layername: '关系数据',
            id: 'relationdata',
    ...
    layers: [{
        ...
        url: createUrlForPoi('editsupport/infor/tile/?', ""),
        clazz: fastmap.mapApi.tileJSON,
        options: {
            layername: 'POI点数据',
            id: 'poiPoint',
...
function transformData(data, url) {
    ...
    if (Object.prototype.toString.apply(data) == '[object Object]') {
        for (var object in data) {
            result = result.concat(parseData(data[object]));
        }
    ...
    $.each(data, function(index, item) {
        ...
        switch (item.t) {
            ...
            case 6: //点限速
                ...
                obj['properties']['markerStyle'] = {};
                obj['properties']['markerStyle']["icon"] = [];
                obj['properties']["featType"] = "RDSPEEDLIMIT";
                if (type == 0) {
            ...
            case 11: //立交
                ...
                rdGscMarkObj['properties']["featType"] = "RDGSC";
...
var RD_LINK_Colors = ['#646464', '#FFAAFF', '#E5C8FF', '#FF6364', '#FFC000', '#0E7892', '#63DC13', '#C89665', '#C8C864', '#000000', '#00C0FF', '#DCBEBE', '#000000', '#7364C8', '#000000', '#DCBEBE'];
...
function transformDataForPoi(data, param) {
    var featArr = [],
        guideLineObj = null,
        linePoint;
    ...
    $.each(data, function(index, item) {
        ...
        obj['properties']["featType"] = item.l;
        obj['properties']['status'] = item.p;
        switch (item.p) {
            case 3: //施工维修
                featArr.pop();
                for (var i = 0; i < 2; i++) {
                    ...
                    obj['properties']['status'] = item.p;
                    if (i == 0) {
                        obj['geometry']['coordinates'] = item.x;
                    } else {
                        obj['geometry']['coordinates'] = item.y;
                    }
                    obj['properties']['markerStyle']["icon"].push(getIconStyle({
                        iconName: '../../../images/poi/map/dot_infor_16.png',
                        row: 0,
                        column: 1,
                        location: obj['geometry']['coordinates']
                    }));
...
function createUrl(url, requestType) {
    var urlObj = {};
    urlObj.url = App.Config.roadUrl + url;
    urlObj.parameter = {
        projectId: App.Config.roadProjectid,
        gap: 80,
        types: requestType.split(',')
    }
    if (requestType == "RDLINK") {
        urlObj.hbaseUrl = App.Config.roadUrl + '/render/obj/getByTileWithGap?';
    }
    return urlObj;
}
/**
 ...
 * @returns {{}}
 */
function createUrlForPoi(url, requestType) {
    var urlObj = {};
    if (requestType != "") {
