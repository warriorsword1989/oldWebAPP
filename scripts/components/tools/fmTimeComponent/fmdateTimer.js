angular.module('lazymodule', []).controller('DateCtrl', ['$scope', '$timeout', '$compile', function ($scope, $timeout, $compile) {
    ...
    $scope.translate = function (arr) {
        ...
        $scope.translateDetail = function (v) { // 验证是否为模糊
            ...
            if (dateSingle.length == 1) { // ex:["(t2t3t4t5t6)"]
                ...
                } else if (weeks_arr.length > 1 && weeks_arr[0].indexOf('t') > -1 && weeks_arr[1].indexOf('t') == -1) {
                    weeks_arr.reverse();
                }
                ...
                if (weeksStr.length >= 1 && weeksStr[0].indexOf(')(') == -1 && mutiDate.length == 1 && mutiDate[0].indexOf('y') == -1 && mutiDate[0].indexOf('{d') == -1) {
                    ...
                    if (weeks_arr[0].indexOf('){d') > -1) {
                        ...
                        detail = '每个星期中' + $scope.weekTranslate(temp[0].split('t')[1]) + '的' + $scope.timeAnalyze(weeks_arr[1].split(')(')[0]) + '到' + $scope.weekTranslate(parseInt(temp[0].split('t')[1]) + parseInt(temp[1].split('d')[1]) + '') + '的' + $scope.timeAnalyze(weeks_arr[1].split(')(')[1]);
                    } else {
                        temp = weeks_arr[1].split('){');
                        detail = '每个星期中' + $scope.weekTranslate(temp[0].split('t')[1]) + '的' + $scope.timeAnalyze(weeks_arr[0].split(')(')[0]) + '到' + $scope.weekTranslate(parseInt(temp[0].split('t')[1]) + parseInt(temp[1].split('d')[1]) + '') + '的' + $scope.timeAnalyze(weeks_arr[0].split(')(')[1]);
                    }
                }
    ...
    $scope.dateEmpty = function (e) {
        ...
        $scope.$emit('get-date', '[]');
        if ($(e.target).parents('.date-well').parent().hasClass('muti-date')) $(e.target).parents('.date-well').parent().attr('date-str', '');
    };
}]);
