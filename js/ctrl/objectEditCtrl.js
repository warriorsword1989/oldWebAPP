...
objectEditApp.controller("normalController", function ($scope,$timeout) {
    ...
    $scope.fmdateTimer = function(str){
        var newDateStr = '';
        /*获取新数据*/
        $scope.$on('newString', function(event,data) {
            newDateStr = data; 
        });
        ...
        $scope.$parent.$parent.getCode = function(){
            $scope.codeOutput = newDateStr;
        }
        $timeout(function(){
...
objectEditApp.controller('DateCtrl', ['$scope','$timeout','$compile', function($scope,$timeout,$compile){
    /*星期*/
    var wks = $scope.wks = {};
    wks.values = [
    ...
    $scope.removeList = function(e){
        ...
        $scope.dateString = ($scope.dateString).replace(/\[\+\[/, "[[");
        $scope.dateString = ($scope.dateString).replace(/\]\+\]/, "]]");
        $scope.$emit('newString', $scope.dateString);
    ...
    $scope.dateValid = function(){
        if(dateWell.find("input[time-type='fix-time']").is(':checked')){
            ...
            switch($scope.dwm.selection.code){
                case 'd':
                    if(!$scope.dBeginDate){
